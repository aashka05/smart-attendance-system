...
async def submit_enrollment(
    ...
    existing = conn.execute(
        "SELECT id, status FROM face_enrollments WHERE student_id = ?",
        (current_user["id"],)
    ).fetchone()
    ...
    if existing:
        conn.execute("""
            ...
            SET face_image_b64 = ?, id_card_image_b64 = ?,
                face_embedding = ?, status = 'pending',
                submitted_at = ?, reviewed_at = NULL, rejection_reason = NULL
            WHERE student_id = ?
        """, (
            req.face_image_b64,
    ...
    conn.execute(
        "UPDATE users SET face_enrolled = 2 WHERE id = ?",
        (current_user["id"],)
    )
...
async def get_enrollment_status(current_user: dict = Depends(get_current_user)):
    ...
    enrollment = conn.execute("""
        ...
        FROM face_enrollments WHERE student_id = ?
...
async def review_enrollment(
    ...
    conn = get_db()
    enrollment = conn.execute(
        "SELECT * FROM face_enrollments WHERE id = ?", (req.enrollment_id,)
    ).fetchone()
    ...
    conn.execute(
        "UPDATE users SET face_enrolled = ? WHERE id = ?",
...
async def check_duplicate_face(
    ...
    all_enrollments = conn.execute("""
        ...
        FROM face_enrollments fe
        LEFT JOIN users u ON fe.student_id = u.id
        WHERE fe.status = 'approved' AND fe.student_id != ?
    ...
    for enrolled in all_enrollments:
        ...
        if sim > 0.6:
            raise HTTPException(
                status_code=400,
                detail="Face already registered to another account"
            )
...
async def get_challenge(
    ...
    """, (req.session_id, current_user["id"])).fetchone()
...
async def verify_face(
    ...
    challenge = conn.execute("""
        ...
        WHERE id = ? AND student_id = ? AND session_id = ? AND used = 0
    """, (req.challenge_id, current_user["id"], req.session_id)).fetchone()
    ...
    if similarity < SIMILARITY_THRESHOLD:
        ...
        raise HTTPException(
            status_code=400,
            detail=f"Face does not match. Similarity: {similarity:.2f}"
        )

    # Mark challenge used
    conn.execute(
        "UPDATE liveness_challenges SET used = 1 WHERE id = ?",
        (req.challenge_id,)
    )
